...
const PostResult = ({ iconKey }) => {
    const downloadPNG = () => {
        const svgUrl = ICON_MAP[iconKey];
        if (!svgUrl) return;

        const img = new Image();
        img.crossOrigin = "anonymous"; // important for canvas export(apparently)
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-8">
            <h2 className="text-[#F4D8B8] uppercase tracking-tight text-4xl mt-10 animate-pulse">Share Badge</h2>
            <div className="flex flex-row items-center space-x-40 justify-center mt-5">
                ...
                <div className="flex flex-col items-center justify-center p-[2px] border border-purple-200 border-[3px] rounded-[10px]">
                    <div className="flex flex-col items-center justify-center pt-5 px-5 pb-2 border border-purple-200 border-[3px] rounded-[10px]">
                        <div className="bg-white rounded border border border-[3px]">
                            <img
                                src={ICON_MAP['qr']}
                                alt={ICON_MAP['qr']}
                        ...
                        <p className="text-white mt-2 font-medium text-[10px]">Scan to share</p>
                    </div>
                </div>
